##### Nichenet Differential NicheNet analysis between conditions of interest
...
DimPlot(bmac, group.by = "CellType") # user adaptation required on own dataset


bmac <- AddMetaData(
  object = bmac,
  metadata = bmac@active.ident,
  col.name = 'CellType')


table(bmac@meta.data$CellType, bmac@meta.data$Condition)


bmac@meta.data$celltype_aggregate = paste(bmac@meta.data$CellType, bmac@meta.data$Condition,sep = "_") # user adaptation required on own dataset
DimPlot(bmac, group.by = "celltype_aggregate")


bmac@meta.data$celltype_aggregate %>% table() %>% sort(decreasing = TRUE)



celltype_id = "celltype_aggregate" # metadata column name of the cell type of interest
seurat_obj = SetIdent(bmac, value = bmac[[celltype_id]])
...
niches = list(
  "osteo_High_niche" = list(
    "sender" = c("CD8 T cells_OA", "NK_OA", "Mature B_OA", "MEGA_OA", "DNTLGALS3_high_OA"),
    "receiver" = c("MSC_OA")),
  "osteo_Low_niche" = list(
    "sender" = c("CD8 T cells_non OA", "NK_non OA" , "Mature B_non OA", "MEGA_non OA", "DNTLGALS3_high_non OA"),
    "receiver" = c("MSC_non OA"))
...
geneset_OA = DE_receiver_processed_targets %>% filter(receiver == niches$osteo_High_niche$receiver & target_score >= lfc_cutoff & target_significant == 1 & target_present == 1) %>% pull(target) %>% unique()
geneset_nonOA = DE_receiver_processed_targets %>% filter(receiver == niches$osteo_Low_niche$receiver & target_score >= lfc_cutoff & target_significant == 1 & target_present == 1) %>% pull(target) %>% unique()
...
geneset_OA %>% setdiff(rownames(ligand_target_matrix))

geneset_nonOA %>% setdiff(rownames(ligand_target_matrix))
...
#Visualization for the other condition: non OA
...
##circos plot prioritizing ligand-receptor pairs
